...
class TernaryHeap
{
    ...
    public:
    /** Constructor **/
        TernaryHeap(int capacity, int numKids)
        {
            currentSize = 0;
            d = numKids;
            size = capacity + 1;
        ...
        TernaryHeap(int* array, int numKids)
        {
            int i = 0;
            while (array[i] != -1)
                i++;
...
int main()
{
    cout<<"Ternary Heap Test\n\n";
    cout<<"Enter size and no of nodes each child has: ";
    int size, num, choice, val;
    cin>>size>>num;
    /** Make object of TernaryHeapHeap **/
    TernaryHeap th(size, num);
    char ch;
